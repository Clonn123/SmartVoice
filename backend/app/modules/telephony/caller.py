...
from app.modules.llm.base import LlmCallContext
from app.modules.llm.factory import get_llm_gateway
from app.modules.llm.context import (
    DEFAULT_REALTIME_PROMPT,
    DEFAULT_REALTIME_SCENARIO,
    build_default_realtime_target,
)
...
class AsteriskCaller:
    ...
    async def init_call(
        ...
        *,
        ...
        self.ctx = CallContext()
        self.ctx.call_task_id = call_task_id
        self.ctx.call_attempt_id = call_attempt_id
        ...
        try:
            ...
            self.ctx.llm_target = target_payload or build_default_realtime_target(
                number
            )
            ...
            try:
                ...
                self.ctx.llm_opening_reply = opening_reply.message.strip() or (
                    "Здравствуйте! Подскажите, пожалуйста, удобно ли вам "
                    "сейчас говорить?"
                )
            except Exception:
    ...
    def cleanup_call(self):
        ...
        if self.pipeline:
            try:
                ...
                print("PIPELINE STOP ERROR:", exc)

            self.pipeline = None

        if ctx.rtp_session:
            try:
                ctx.rtp_session.stop()
            except Exception as exc:
                print("RTP STOP ERROR:", exc)

        ctx.active = False
        ctx.channel_id = None
        ctx.external_media_channel_id = None
        ctx.bridge = None
        ctx.rtp_session = None
